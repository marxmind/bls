 ...
 * @since 05/04/2017
 * @version 1.0
 * This functionality has been changed. Please check Events class
 */
...
public class SchedulerBean implements Serializable{
    ...
    public void init() {
       eventModel = new DefaultScheduleModel();
        ...
        try{
        List<Reservation> val = ReservationXML.readReservationXML();
        if(val.size()>0){
        for(Reservation rs : val){
        	ScheduleEvent eve = new DefaultScheduleEvent();
        	eve = loadSchedule(rs);
        	eventModel.addEvent(eve);
        	eve.setId(rs.getId());
        }
        rvs = new ArrayList<Reservation>();
        Map<String, Reservation> scs = Collections.synchronizedMap(new HashMap<String, Reservation>());
        for(Reservation rc : val){
        	
        	String dateFrom = rc.getStartDate().split(",")[0];
        	String hFrom = rc.getStartDate().split(",")[1];
        	String dateTo = rc.getEndDate().split(",")[0];
        	String hTo = rc.getEndDate().split(",")[1];
        	
        	try{
        		/*int fhour = Integer.valueOf(hFrom.split(":")[0]);
        		if(fhour>=13){
        			DateUtils.timeTo12Format(time, isIncludePM)
        		}*/
        		hFrom = hFrom.split(" ")[0];
        		hTo = hTo.split(" ")[0];
        	...
        	int lenWords = rc.getDescription().length();
        	String desc = "";
        	if(lenWords>=10){
        		desc = rc.getDescription().substring(0, 10);
        		desc +="...";
        	}else{
        	...
        	if(fmonth.equalsIgnoreCase(tmonth)){
        		if(fday.equalsIgnoreCase(tday)){
        			rc.setMemos(fmonth + " " + fday + " "+ tyear +" ("+ hFrom +"-"+ hTo +"): " +desc);
        			scs.put(fmonth+fday+fyear+"-"+tmonth+tday+tyear, rc);
        		}else{
        			if(fyear.equalsIgnoreCase(tyear)){
        				rc.setMemos(fmonth + " " + fday + "-" + tday + " " + tyear +" ("+ hFrom +"-"+ hTo +"): " +desc);
        				scs.put(fmonth+fday+fyear+"-"+tmonth+tday+tyear, rc);
        			}else{
        				rc.setMemos(fmonth + " " + fday + " "+ fyear +"-" + tday + " " + tyear +" ("+ hFrom +"-"+ hTo +"): " +desc);
        				scs.put(fmonth+fday+fyear+"-"+tmonth+tday+tyear, rc);
        			}
        		}
        	}else{
        		if(fyear.equalsIgnoreCase(tyear)){
        			rc.setMemos(fmonth + " " + fday + "-"+ tmonth+ " " + tday + " " + tyear +" ("+ hFrom +"-"+ hTo +"): " +desc);
        			scs.put(fmonth+fday+fyear+"-"+tmonth+tday+tyear, rc);
        		}else{
        			rc.setMemos(fmonth + " " + fday + " "+ fyear + "-"+ tmonth+ " " + tday + " " + tyear +" ("+ hFrom +"-"+ hTo +"): " +desc);
        			scs.put(fmonth+fday+fyear+"-"+tmonth+tday+tyear, rc);
        		}
        	}
        ...
        for(Reservation rs : sorted.values()){
        	rvs.add(rs);
        }
